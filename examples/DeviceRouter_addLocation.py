...
# written by Bryan Irvine @ Zenoss                  #
#####################################################
...
usage = '%s <new_location>'  % (sys.argv[0])
...
def buildArgs():
    ...
    if len(sys.argv) != 2:
        ...
        try:
            contextUid, location = sys.argv[1].rsplit('/', 1)
        except:
            fail()
    data = {'id': location,
            'description': '',
...
def recursePath(data):
    ...
    count = 0
    buildOrgs = '/zport/dmd/Locations'
    while count < len(newOrgs):
        buildOrgs += "/" + newOrgs[count]
        count += 1
        contextUid, location = buildOrgs.rsplit('/', 1)
        data = {'id': location,
                'description': '',
                'type': 'organizer',
                'address': '',
                'contextUid': contextUid}
        api_response = addLocation(data) 
        print api_response
